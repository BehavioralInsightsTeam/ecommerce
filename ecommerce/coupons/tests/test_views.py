...
class EnrollmentCodeCsvViewTests(TestCase):
    ...
    def test_invalid_order_number(self):
        """ Verify an error is returned for an invalid order number. """
        response = self.client.get(reverse(self.path, args=['INVALID']))
        self.assertEqual(response.context['error'], _('Invalid order.'))

    def test_invalid_user(self):
        """ Verify an error is returned if a different user requests the CSV. """
        order = OrderFactory()
